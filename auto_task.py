...
from sync_app_data import sync_data_to_app_table # [启动] 新增导入
...
def daily_quant_job():
    ...
    try:
        ...
        # [启动] 第三步：自动格式化 App 所需的展示数据
        print("️ 步骤 3/3: 正在同步 App 视图数据...")
        sync_data_to_app_table()

        print("[完成] 今日量化任务圆满完成！各池子数据已更新。")
        
    except Exception as e:
        print(f"[失败] 自动任务执行发生严重错误: {e}")
...
# ==========================================
# schedule.every().day.at("00:47").do(daily_quant_job)

if __name__ == "__main__":
    print("[守护] 量化无人值守守护进程已启动...")
    print("⏳ 系统正在后台静默运行，每天 15:35 将自动执行【拉取数据 + 策略流转】。")
    print("[提示] 请保持此终端不被关闭 (你可以将其最小化)。")
